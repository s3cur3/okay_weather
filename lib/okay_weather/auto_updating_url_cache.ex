defmodule OkayWeather.AutoUpdatingUrlCache do
  @moduledoc """
  ...
  def start_link(name, url_generator, update_ms \\ :timer.minutes(3))
  ...
  def get(cache_name) when is_atom(cache_name),
    do: GenServer.call(via_tuple(cache_name), :lookup, @timeout_ms)

  ################ Server Implementation ################
  @impl GenServer
  def init({url_generator, update_ms})
      when is_function(url_generator) and is_integer(update_ms) do
  ...
  defp content_changed(%HTTPoison.Response{body: old}, %HTTPoison.Response{body: new}),
    do: old != new
